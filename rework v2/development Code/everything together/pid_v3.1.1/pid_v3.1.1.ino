  ...
  if (interval >= 3000) {
    ...
    switch (currentChannel) {
      case 0:
        ...
        channel1 = (interval >= 50) ? constrain(interval - 50, MIN_THROTTLE, MAX_THROTTLE) : MIN_THROTTLE;
        break;
      case 1: channel2 = constrain(interval, MIN_THROTTLE, MAX_THROTTLE); break;
      case 2: channel3 = constrain(interval, MIN_THROTTLE, MAX_THROTTLE); break;
      case 3: 
        // Throttle needs special handling to prevent going below MIN_THROTTLE
        channel4 = constrain(interval, MIN_THROTTLE, MAX_THROTTLE); 
        break;
      case 4: channel5 = constrain(interval, MIN_THROTTLE, MAX_THROTTLE); break;
      case 5: channel6 = constrain(interval, MIN_THROTTLE, MAX_THROTTLE); break;
      case 6: channel7 = constrain(interval, MIN_THROTTLE, MAX_THROTTLE); break;
      case 7: channel8 = constrain(interval, MIN_THROTTLE, MAX_THROTTLE); break;
    }
    currentChannel++;
  }
}
...
void updateAndSendPPM() {
  ...
  if (xSemaphoreTake(ppmMutex, portMAX_DELAY) == pdTRUE) {
    // Ensure channel4 (throttle) never goes below MIN_THROTTLE
...
void controlHeight() {
  ...
  // Throttle anpassen basierend auf PID-Output mit Sicherheitsgrenze
  if (xSemaphoreTake(ppmMutex, portMAX_DELAY) == pdTRUE) {
    // Limit the negative adjustment to prevent throttle dropping too low
    int adjustment = (int)z_output;
    if (channel4 + adjustment < MIN_THROTTLE) {
      adjustment = MIN_THROTTLE - channel4;
    }
    channel4 = constrain(channel4 + adjustment, MIN_THROTTLE, MAX_THROTTLE);
    xSemaphoreGive(ppmMutex);
  }
}
...
// --- Tasks ---
void ppm_task(void *pvParameters) {
  vTaskDelay(pdMS_TO_TICKS(100)); // in jedem Task ganz am Anfang
...
void setup() {
  ...
  // Set initial channel values to safe defaults
  channel1 = 1500; // Roll center
  channel2 = 1500; // Pitch center
  channel3 = 1500; // Yaw center
  channel4 = MIN_THROTTLE; // Throttle minimum
  channel5 = 1500;
  channel6 = 1500;
  channel7 = 1500;
  channel8 = 1500;
  ...
  if (ppmMutex == NULL || sensorMutex == NULL) {
    Serial.println("Fehler beim Erstellen der Mutexes!");
    while (true); // Halt, um Problem zu sehen
  }

  // Print initial channel values
  debugPrint("Initial channel values:", 1);
  debugPrintValue("Throttle (ch4)", channel4, 1);
